...
### Deprecated
- `registerDeviceToken()` / `unregisterDeviceToken()` now do nothing but log.
  Hive no longer sends push notifications itself: an app that already has push
  ends up with two systems notifying one phone, and holding merchants' signing
  keys was a liability not worth carrying. Notify from your own backend
...
### Added
- `registerDeviceToken()` / `unregisterDeviceToken()` — Hive sends the push
  itself once your team pastes the app's Firebase credentials into the
  dashboard. No endpoint of yours, no polling, nothing to configure on your
  domain. Keyed on the visitor token, so it works for customers who never
  signed in.
